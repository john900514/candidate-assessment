<template>
    <div class="w-full h-2/5 bg-slate-600 pt-[30%] md:pt-[15%] banner-bg" :style="bannerImg()">
        <div class="text-center pb-36">
            <h2 class="text-4xl md:text-7xl uppercase text-secondary font-black tracking-wider">{{ club['club_name']}}</h2>
            <p class="tracking-wider uppercase pt-4 font-bold text-black">
                <span>{{ club['address1']+', ' }}</span>
                <br class="sm:inline-block md:hidden"/>
                <span>{{ club['city'] + ','}} {{ club['state'] }} {{ club['zip_code'] + ' ' }} </span>
                <br class="sm:inline-block md:hidden"/>
    ...
    <club-hours :hours="hoursTranslated" v-if="hoursTranslated.length > 0"></club-hours>
</template>
